...
# ARGUMENTS #
complaints_data = "covid-studie-rachel-klachten.csv"
meds_data = "covid-studie-rachel-klinische-medicatie.csv"
lab_data = "covid-studie-rachel-lab.csv"
admission_data = "covid-studie-rachel-opname-delen-admission-parts.csv"
overview_data = "covid-studie-rachel-overview.csv"
q10_data = "covid-studie-rachel-q-10.csv"
q11_data = "covid-studie-rachel-q-11.csv"
vitals_data = "covid-studie-rachel-vitale-metingen.csv"
vms_data = "covid-studie-rachel-vms-scores.csv"

patient_data = "Stats/Patient_Statistics_Haga.csv"
#############


def load_data(patient_df, c, m, l, a, o, q10, q11, vi, vms):
    complaints_data = pd.read_csv(c, delimiter=',', quotechar='"')
    complaints_data = complaints_data[complaints_data['pseudo_id'].isin(patient_df.index)]
    meds_data = pd.read_csv(m, delimiter=',', quotechar='"')
    meds_data = meds_data[meds_data['pseudo_id'].isin(patient_df.index)]
    lab_data = pd.read_csv(l, delimiter=',', quotechar='"')
    lab_data = lab_data[lab_data['pseudo_id'].isin(patient_df.index)]
    admission_data = pd.read_csv(a, delimiter=',', quotechar='"')
    admission_data = admission_data[admission_data['pseudo_id'].isin(patient_df.index)]
    overview_data = pd.read_csv(o, delimiter=',', quotechar='"')
    overview_data = overview_data[overview_data['pseudo_id'].isin(patient_df.index)]
    q10_data = pd.read_csv(q10, delimiter=',', quotechar='"')
    q10_data = q10_data[q10_data['pseudo_id'].isin(patient_df.index)]
    q11_data = pd.read_csv(q11, delimiter=',', quotechar='"')
    q11_data = q11_data[q11_data['pseudo_id'].isin(patient_df.index)]
    vitals_data = pd.read_csv(vi, delimiter=',', quotechar='"')
    vitals_data = vitals_data[vitals_data['pseudo_id'].isin(patient_df.index)]
    vms_data = pd.read_csv(vms, delimiter=',', quotechar='"')
    vms_data = vms_data[vms_data['pseudo_id'].isin(patient_df.index)]
...
def reformat_lab(df, patient_data, symp_time=True):
    ...
    for row in range(len(df.index)):
        ...
        labcode = str(df.at[row, 'lab_measurement_id'])[-8:]

        if labcode in list(lab_key_df[~pd.isna(lab_key_df['Feature Name'])]['Haga Code']):
            ...
            feature = lab_key_df.at[lab_key_df[lab_key_df['Haga Code'] == labcode].index[0], 'Feature Name']
...
def reformat_admission(df, patient_data, symp_time=True):
    ...
    # New empty output df
    if symp_time:
        o_df = pd.DataFrame(columns=['pseudo_id', 'start_time_symp', 'end_time_symp', 'start_date_adm', 'end_date_adm', 'admission_location', 'admission_specialism', 'provider_position'])
    else:
        o_df = pd.DataFrame(columns=['pseudo_id', 'start_date_adm', 'end_date_adm', 'admission_location', 'admission_specialism', 'provider_position'])
    ...
    for row in range(len(df.index)):
        ...
        pseudo_id = df['pseudo_id'].iloc[row]
        if symp_time:
            start_date = abs_to_rel_date(patient_data['symptoms_date'].loc[pseudo_id], df.at[row, 'opname_delen_admission_parts_start_date'])
            end_date = abs_to_rel_date(patient_data['symptoms_date'].loc[pseudo_id], df.at[row, 'opname_delen_admission_parts_end_date'])
        ...
        if symp_time:
            o_df = o_df.append({'pseudo_id': pseudo_id, 'start_time_symp': start_date, 'end_time_symp': end_date, 'start_time_adm': start_date_adm,
                                'end_time_adm': end_date_adm, 'admission_location': adm_loc, 'admission_specialism': adm_spec}
                               , ignore_index=True)
        else:
            o_df = o_df.append({'pseudo_id': pseudo_id, 'start_time_adm': start_date_adm, 'end_time_adm': end_date_adm, 'admission_location': adm_loc,
                                'admission_specialism': adm_spec}
                               , ignore_index=True)
...
def reformat_vms(df, patient_data, symp_time=True):
    ...
    # New empty output df
    o_df = pd.DataFrame(columns=['pseudo_id', 'time', 'time_adm'])
    o_row = - 1
    pseudo_id = 0
    o_time = 0
    ...
    for row in range(len(df.index)):
        ...
        for col in ['vms_scores_form_entries_value_text', 'vms_scores_form_entries_value_multiple_choice',
            ...
            if not pd.isna(df.at[row, col]):
                value = df.at[row, col]
                break
        time = abs_to_rel_date(patient_data['symptoms_date'].loc[pseudo_id], o_time)
        time_adm = abs_to_rel_date(patient_data['admitted_date'].loc[pseudo_id], o_time)
...
def reformat_vitals(df, patient_data, symp_time=True):
    ...
    for row in range(len(df.index)):
        ...
        feature = vitals_key_df.at[vitals_key_df[vitals_key_df['Haga Description'] == descr].index[0], 'Feature Name']
...
complaints_data, meds_data, lab_data, admission_data, overview_data, q10_data, q11_data, vitals_data, vms_data = load_data(patient_data, complaints_data,
                 meds_data, lab_data, admission_data, overview_data, q10_data, q11_data, vitals_data, vms_data)

# Process + save all individual data files
# pro_complaints = reformat_complaints(complaints_data, patient_data)
# pro_complaints.to_csv('Processed_Data/complaints_processed.csv', index=False)
...
# pro_overview.to_csv('Processed_Data/overview_processed.csv', index=False)
...
# pro_vitals.to_csv('Processed_Data/vitals_processed.csv', index=False)

pro_lab = reformat_lab(lab_data, patient_data, symp_time=False)
pro_lab.to_csv('Processed_Data/lab_processed.csv', index=False)

# pro_vms = reformat_vms(vms_data, patient_data, symp_time=False)
# pro_vms.to_csv('Processed_Data/vms_processed.csv', index=False)
