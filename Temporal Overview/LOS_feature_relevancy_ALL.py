...
haga_patient_df = exclude(f"{haga_path}Stats/Patient_Statistics_Haga.csv", f"{haga_path}Stats/Patient_Statistics_Haga.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
haga_patient_df.set_index('pseudo_id', inplace=True)
lumc_patient_df = exclude(f"{lumc_path}Stats/Patient_Statistics_LUMC.csv", f"{lumc_path}Stats/Patient_Statistics_LUMC.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
lumc_patient_df.set_index('pseudo_id', inplace=True)
# Load vitals data
haga_all_vitals_df = exclude(f"{haga_path}Processed_Data/vitals_processed.csv", f"{haga_path}Stats/Patient_Statistics_Haga.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
lumc_all_vitals_df = exclude(f"{lumc_path}Processed_Data/vitals_processed.csv", f"{lumc_path}Stats/Patient_Statistics_LUMC.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
# Load Lab data
haga_all_lab_df = exclude(f"{haga_path}Processed_Data/lab_processed.csv", f"{haga_path}Stats/Patient_Statistics_Haga.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
lumc_all_lab_df = exclude(f"{lumc_path}Processed_Data/lab_processed.csv", f"{lumc_path}Stats/Patient_Statistics_LUMC.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
# Load meds data
haga_all_meds_df = exclude(f"{haga_path}Processed_Data/medication_processed.csv", f"{haga_path}Stats/Patient_Statistics_Haga.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
lumc_all_meds_df = exclude(f"{lumc_path}Processed_Data/medication_processed.csv", f"{lumc_path}Stats/Patient_Statistics_LUMC.csv", treatment_limit=False, dis_other_hospital=False, symptom_onset=False, icu=False)
...
rows = ['Total patients', 'Age', 'Female (%)', 'Directly adm. to ICU (%)', 'Adm. to ICU (%)', 'Diseased (%)', 'Treat. Limit. (%)',
        ...
        'Dest.: usual place of residency (%)', 'Dest.: care home (%)', 'Dest.: institution (%)', 'Dest.: other hospital (%)', 'Dest.: deceased (%)',
...
for i, los in enumerate(LOS_groups):
    ...
    # Dir. adm. to ICU
    o_df.loc['Directly adm. to ICU (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['admitted_loc'] == 'IC')].index)
                                                      / tot_patients) * 100, 1)
    ...
    # Origins
    o_df.loc['Origin: usual place of residency (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['admission_origin'] == 'Eigen woonomgeving')].index)
                                                                / tot_patients) * 100, 1)
    o_df.loc['Origin: care home (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['admission_origin'] == 'Verpleeg-/verzorgingshuis')].index)
                                                                       / tot_patients) * 100, 1)
    o_df.loc['Origin: institution (other) (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['admission_origin'] == 'Instelling (anders)')].index)
                                                                 / tot_patients) * 100, 1)

    # Destinations
    o_df.loc['Dest.: usual place of residency (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['discharge_dest'] == 'Eigen woonomgeving')].index)
                                                              / tot_patients) * 100, 1)
    o_df.loc['Dest.: care home (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['discharge_dest'] == 'Verpleeg- / verzorgingshuis')].index)
                                                                       / tot_patients) * 100, 1)
    o_df.loc['Dest.: institution (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['discharge_dest'].str.startswith('Instelling'))].index)
                                                      / tot_patients) * 100, 1)
    o_df.loc['Dest.: other hospital (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['discharge_dest'].str.contains('ziekenhuis', case=False))].index)
                                                            / tot_patients) * 100, 1)
    o_df.loc['Dest.: deceased (%)'].iloc[i] = round((len(patient_df[(patient_df['hospitalisation_time'] <= los) & (patient_df['discharge_dest'].str.startswith('Overleden'))].index)
                                                     / tot_patients) * 100, 1)
    # Supplementary O2
